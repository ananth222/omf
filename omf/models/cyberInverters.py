...
def work(modelDir, inputDict):
	...
	def convertInputs():
		...
		#create device_inputs.txt file in folder
		with open(pJoin(modelDir,"PyCIGAR_inputs","device_inputs.txt"),"w", newline='') as dvc_stream:
			dvc_stream.writelines(inputDict['deviceFileContent'])
	...
	# Aggregate up the timestamps: # NOTE: only seconds is supported, so this functionality has been redacted
	#if level=='days':
...
def new(modelDir):
	...
	with open(pJoin(omf.omfDir, "static", "testFiles", "pyCIGAR", ckt_dir, dvc_fn)) as dvc_stream:
		dvc_ins = dvc_stream.read()
